...
=head2 DOES
  ...
  $bool = $class->DOES( $role_name );

Used to fake C<Role::Tiny> to make L<Beam::Wire> pass the C<service> and
C<name> arguments to the C<load> method. Returns C<true> when C<$role_name>
equals C<Beam::Service>.

=cut

sub DOES( $class, $role_name ) {
    return ($role_name eq 'Beam::Service');
}

=head2 load( directories => @directories, lifecycle => $lifecycle, container => $container, ... )
...
makes less sense for persistent environments such as the Plack server.
...
sub load($class, :$container, :$directories, :$lifecycle = undef, %) {
    my %files = map {
    ...
    my $instance = LedgerSMB::Workflow::Factory->instance( $container );
