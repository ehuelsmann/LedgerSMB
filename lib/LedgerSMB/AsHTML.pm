## no critic (RequireFilenameMatchesPackage,ProhibitEvilModules)
...
function toggleThingTR(ref, type) {
...
function toggleArgumentsTR(ref) {
 toggleThingTR(ref, 'arguments');
}
function toggleLexicalsTR(ref) {
 toggleThingTR(ref, 'lexicals');
}
</script>
</head>
<body>
...
sub _build_arguments {
    ...
    for my $idx (0 .. @args - 1) {
        ...
        $html .= qq{<tr>};
        $html .= qq{<td class="variable">};
		$html .= qq(<a class="toggle" href="javascript:toggleArgumentsTR('$ref-$v')">);
        $html .= qq{\$_[$idx]};
		$html .= qq{</a>};
        $html .= qq{</td>};
        $html .= qq{<td class="value">};
        $html .= qq{<a id="arguments-$ref-$v">};
		$html .= encode_html($dump) . qq{</a>};
        $html .= qq{</td>};
        $html .= qq{</tr>};
		$v++;
    }
...
sub _build_lexicals {
    ...
    for my $var (sort keys %$lexicals) {
        ...
        $html .= qq{<tr>};
        $html .= qq{<td class="variable">};
		$html .= qq(<a class="toggle" href="javascript:toggleLexicalsTR('$ref-$v')">);
		$html .= encode_html($var);
		$html .= qq{</a>};
		$html .= qq{</td>};
        $html .= qq{<td class="value">};
        $html .= qq{<a id="lexicals-$ref-$v">};
		$html .= encode_html($dump) . qq{</a>};
        $html .= qq{</td>};
        $html .= qq{</tr>};
		$v++;
