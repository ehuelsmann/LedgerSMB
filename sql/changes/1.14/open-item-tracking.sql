...
create table open_item (
  ...
  opening_entry_id int references acc_trans(entry_id) on delete cascade
  );
...
comment on column open_item.opening_entry_id is
  $$The entry_id of the journal line creating the opening balance. $$;
comment on column acc_trans.open_item_id is
  $$The open item this journal line is linked to; this indicates an allocation (reduction) of the open item, unless the entry_id of this line item is equal to the opening_entry_id of the open item, in which case it is the opening balance. $$;
...
insert into open_item (
  item_number, item_type, account_id, opening_entry_id
)
select invnumber, lower(al.description), chart_id, min(entry_id)
  from acc_trans
