...
BEGIN
  ...
  INSERT INTO transactions (id, table_name, transdate,
                            approved,
                            reference, trans_type_code, entered_by, workflow_id)
              OVERRIDING SYSTEM VALUE
  SELECT epi.trans_id, 'payment', in_payment_date,
         CASE WHEN t_voucher_id IS NULL THEN true ELSE false END, -- approved
         epi.reference, 'pa', person__get_my_entity_id(), null -- workflow_id
...
BEGIN
  ...
  VALUES ('payment', in_datepaid, coalesce(in_approved, true),
          ...
          'pa', person__get_my_entity_id(), NULL)
