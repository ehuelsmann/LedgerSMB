...
CREATE OR REPLACE FUNCTION file__store(in_content bytea,
  ...
  $$
  WITH insert_new AS (
    insert into file_base (content, mime_type_id)
    values (in_content, in_mime_type_id)
           on conflict (content) do nothing
           returning id;
  ),
  SELECT id FROM (
    select id from insert_new
     union all
    select id from file_base where sha_hash = sha512(in_content)
  ) x
  limit 1;
  $$;

COMMENT ON file__store(bytea, int) IS
  $$ $$;
