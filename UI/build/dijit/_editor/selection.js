//>>built
define("dijit/_editor/selection", ["dojo/dom", "dojo/_base/lang", "dojo/sniff", "dojo/_base/window", "../main"], function(k, m, l, c, g) {
  var h = {getType:function() {
    ...
    if(c.doc.getSelection) {
      ...
      if(a && a.rangeCount) {
        ...
        for(b = 0;b < a.rangeCount;b++) {
          var e = a.getRangeAt(b).cloneContents(), f = c.doc.createElement("div");
          f.appendChild(e);
          d += f.innerHTML
        }
        return d
    ...
    if("control" == g._editor.selection.getType()) {
      ...
      if((a = c.doc.selection.createRange()) && a.item) {
        return c.doc.selection.createRange().item(0)
      }
    }
    return null
    ...
    if("control" == g._editor.selection.getType()) {
      ...
      if(c.doc.getSelection) {
        ...
        return a = c.doc.selection.createRange(), a.collapse(!0), a.parentElement()
      }
    }
    return null
    ...
    if(c.doc.getSelection) {
      ...
      b = c.doc.selection.createRange(), b.collapse(a), b.select()
    }
  }, remove:function() {
    ...
    if(c.doc.getSelection) {
      ...
      l("opera") ? (d = e.rangeCount ? e.getRangeAt(0) : d.createRange(), d.setStart(a, 0), d.setEnd(a, 3 == a.nodeType ? a.length : a.childNodes.length), e.addRange(d)) : e.selectAllChildren(a)
    }else {
      if(d = a.ownerDocument.body.createTextRange(), d.moveToElementText(a), !b) {
        try {
          d.select()
        }catch(f) {
        }
    ...
    var d;
    a = k.byId(a);
    var e = a.ownerDocument, f = c.global;
    if(e.getSelection) {
      f = f.getSelection(), d = e.createRange(), f.removeAllRanges && (l("opera") && f.getRangeAt(0) && (d = f.getRangeAt(0)), d.selectNode(a), f.removeAllRanges(), f.addRange(d))
    }else {
      try {
        var g = a.tagName ? a.tagName.toLowerCase() : "";
    ...
    if(a) {
      ...
      if(c.doc.getSelection) {
        ...
        if(e && e.compareBoundaryPoints && d.createRange) {
          try {
            if(b = d.createRange(), b.setStart(a, 0), 1 === e.compareBoundaryPoints(e.START_TO_END, b)) {
              return!0
            }
          }catch(g) {
          }
        ...
        if(e && b && 1 === e.compareEndPoints("EndToStart", b)) {
          return!0
        }
      }
    }
  ...
  m.setObject("dijit._editor.selection", h);
  return h
});
