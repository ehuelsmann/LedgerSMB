  ...
  var m = function(f) {
      ...
      if(d.getSelection) {
        ...
        if(a && a.rangeCount) {
          ...
          for(b = 0;b < a.rangeCount;b++) {
            var e = a.getRangeAt(b).cloneContents(), g = d.createElement("div");
            g.appendChild(e);
            c += g.innerHTML
          }
          return c
    ...
    this.getParentElement = function() {
      if("control" == this.getType()) {
        ...
        if(d.getSelection) {
          ...
          return a = d.selection.createRange(), a.collapse(!0), a.parentElement()
        }
      }
      return null
    };
    ...
    this.collapse = function(a) {
      if(d.getSelection) {
        ...
        b = d.selection.createRange(), b.collapse(a), b.select()
      }
    };
    ...
    this.selectElementChildren = function(a, b) {
      ...
      if(d.getSelection) {
        ...
        k("opera") ? (c = e.rangeCount ? e.getRangeAt(0) : d.createRange(), c.setStart(a, 0), c.setEnd(a, 3 == a.nodeType ? a.length : a.childNodes.length), e.addRange(c)) : e.selectAllChildren(a)
      }else {
        if(c = a.ownerDocument.body.createTextRange(), c.moveToElementText(a), !b) {
          try {
            c.select()
          }catch(g) {
          }
    ...
    this.selectElement = function(a, b) {
      ...
      if(d.getSelection) {
        ...
        try {
          e = a.tagName ? a.tagName.toLowerCase() : "", c = "img" === e || "table" === e ? l.body(d).createControlRange() : l.body(d).createRange(), c.addElement(a), b || c.select()
        }catch(f) {
      ...
      if(a) {
        ...
        if(d.getSelection) {
          ...
          if(c && c.compareBoundaryPoints && d.createRange) {
            try {
              if(b = d.createRange(), b.setStart(a, 0), 1 === c.compareBoundaryPoints(c.START_TO_END, b)) {
                return!0
              }
            }catch(g) {
            }
          ...
          if(c && b && 1 === c.compareEndPoints("EndToStart", b)) {
            return!0
          }
        }
      }
    ...
    this.getBookmark = function() {
      var a, b, c = d.selection, e = r.curNode;
      if(d.getSelection) {
        if(c = f.getSelection()) {
          if(c.isCollapsed) {
            if(a = e ? e.tagName : "") {
              if(a = a.toLowerCase(), "textarea" == a || "input" == a && (!e.type || "text" == e.type.toLowerCase())) {
                return c = {start:e.selectionStart, end:e.selectionEnd, node:e, pRange:!0}, {isCollapsed:c.end <= c.start, mark:c}
              }
            }
            a = {isCollapsed:!0};
            c.rangeCount && (a.mark = c.getRangeAt(0).cloneRange())
          }else {
            b = c.getRangeAt(0), a = {isCollapsed:!1, mark:b.cloneRange()}
          }
        }
      }else {
        if(c) {
          ...
          if(e && a && ("button" == a || "textarea" == a || "input" == a)) {
            if(c.type && "none" == c.type.toLowerCase()) {
              return{isCollapsed:!0, mark:null}
            }
            b = c.createRange();
            return{isCollapsed:b.text && b.text.length ? !1 : !0, mark:{range:b, pRange:!0}}
          }
          a = {};
          try {
          ...
          if("CONTROL" == c.type.toUpperCase()) {
            if(b.length) {
              ...
              c = 0;
              for(e = b.length;c < e;) {
                a.mark.push(b.item(c++))
              }
            }else {
              a.isCollapsed = !0, a.mark = null
            }
          }else {
            a.mark = b.getBookmark()
          }
        }
      }
      return a
    ...
    this.moveToBookmark = function(a) {
      if(a = a.mark) {
        if(d.getSelection) {
          var b = f.getSelection();
          b && b.removeAllRanges && (a.pRange ? (b = a.node, b.selectionStart = a.start, b.selectionEnd = a.end) : (b.removeAllRanges(), b.addRange(a)))
        }else {
          if(d.selection && a) {
            var c;
