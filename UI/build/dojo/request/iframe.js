//>>built
define("dojo/request/iframe", "module require ./watch ./util ./handlers ../_base/lang ../io-query ../query ../has ../dom ../dom-construct ../_base/window ../NodeList-dom".split(" "), function(B, C, D, r, E, x, z, F, s, A, t, d) {
  function G(a) {
  ...
  function I(a, b) {
    if(!b) {
      try {
        var f = a.options, k = c.doc(c._frame), e = f.handleAs;
        if("html" !== e) {
          if("xml" === e) {
            if("html" === k.documentElement.tagName.toLowerCase()) {
              F("a", k.documentElement).orphan();
              var d = k.documentElement.innerText, d = d.replace(/>\s+</g, "\x3e\x3c");
              a.text = x.trim(d)
            }else {
              a.data = k
            }
  ...
  function c(a, b, f) {
    var d = r.parseArgs(a, r.deepCreate(K, b), !0);
    a = d.url;
    b = d.options;
    if("GET" !== b.method && "POST" !== b.method) {
      throw Error(b.method + " not supported by dojo/request/iframe");
    }
    c._frame || (c._frame = c.create(c._iframeName, y + "();"));
    a = r.deferred(d, null, G, H, I, J);
  ...
  var u = B.id.replace(/[\/\.\-]/g, "_"), y = u + "_onload";
  d.global[y] || (d.global[y] = function() {
    var a = c._currentDfd;
    if(a) {
      var b = A.byId(a.response.options.form) || a._tmpForm;
      if(b) {
        for(var f = a._contentToClean, d = 0;d < f.length;d++) {
          for(var e = f[d], l = 0;l < b.childNodes.length;l++) {
            var g = b.childNodes[l];
            if(g.name === e) {
              t.destroy(g);
              break
            }
  ...
  c.create = function(a, b, c) {
    ...
    c || (s("config-useXDomain") && s("config-dojoBlankHtmlUrl"), c = s("config-dojoBlankHtmlUrl") || C.toUrl("dojo/resources/blank.html"));
    ...
    try {
      if(!c._currentDfd && c._dfdQueue.length) {
        ...
        if(!a || a.canceled || a.isCanceled && a.isCanceled()) {
          ...
          var b = a.response, f = b.options, k = a._contentToClean = [], e = A.byId(f.form), l = r.notify, g = f.data || null, n;
          !a._legacy && "POST" === f.method && !e ? e = a._tmpForm = t.create("form", {name:u + "_form", style:{position:"absolute", top:"-1000px", left:"-1000px"}}, d.body()) : "GET" === f.method && (e && -1 < b.url.indexOf("?")) && (n = b.url.slice(b.url.indexOf("?") + 1), g = x.mixin(z.queryToObject(n), g));
          if(e) {
            if(!a._legacy) {
              var h = e;
              do {
                h = h.parentNode
              }while(h && h !== d.doc.documentElement);
              h || (e.style.position = "absolute", e.style.left = "-1000px", e.style.top = "-1000px", d.body().appendChild(e));
            ...
            if(a._legacy) {
              if(!m || !m.value) {
                m ? m.value = f.method : e.setAttribute("method", f.method)
              }
            }else {
              a._originalMethod = m ? m.value : null, m ? m.value = f.method : e.setAttribute("method", f.method)
            }
            ...
            f = "", b.options.data && (f = b.options.data, "string" !== typeof f && (f = z.objectToQuery(f))), h = b.url + (-1 < b.url.indexOf("?") ? "\x26" : "?") + f, l && l.emit("send", b, a.promise.cancel), c._notifyStart(b), c.setSrc(c._frame, h, !0)
          }
        }
      }
    }catch(s) {
